...
def generateneighborsolution(solution):
    warehouse2 = copy.deepcopy(warehouse)  # Create a deep copy of warehouse
    degree_of_destruction = 10
    randomindex = np.random.randint(len(solution))
    newsolution = solution[:randomindex] + solution[randomindex + degree_of_destruction:]
    x = randomindex 

    while len(newsolution) < iterations: 
    ...
    new_cost = sum(costs)  # Sum the costs to get the new cost
    return newsolution, new_cost
...
newsolution, newcost = generateneighborsolution(solution)
print(solution)
print(newcost)
print(newsolution)
